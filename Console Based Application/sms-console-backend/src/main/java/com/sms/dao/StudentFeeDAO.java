...
//Author: Jothika R
/*
* This class for the query logic of allocating fees, getting fees for students, update fees.
*/
	...
	public List<StudentFee> getFeesForStudent(long studentId) {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new StudentFee(rs.getLong("id"), rs.getLong("student_id"), rs.getLong("fee_structure_id"),
						rs.getString("status"), paymentId));
			}
