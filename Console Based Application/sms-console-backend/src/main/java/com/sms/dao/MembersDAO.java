	...
	public boolean addMember(User user) {
		try {
			...
			if (user instanceof Student) {
				...
				//Down casting concept used here.
		...
		try {
			...
			while (rs.next()) {
				...
				if ("STUDENT".equalsIgnoreCase(dbRole)) {
					...
					if (rs2.next()) {
						members.add(new Student(rs.getLong("id"), rs.getString("name"), rs.getString("email"),
								rs.getString("password"), dbRole, rs2.getInt("age"), rs2.getString("address"),
								rs2.getString("parent_email"), rs2.getLong("class_id")));
					}
					...
					if (rs2.next()) {
						members.add(new Teacher(rs.getLong("id"), rs.getString("name"), rs.getString("email"),
								rs.getString("password"), dbRole, rs2.getString("phone_number"),
								rs2.getLong("subject_id"), rs2.getLong("class_id")));
					}
					...
					if (rs2.next()) {
						...
						members.add(new Parent(rs.getLong("id"), rs.getString("name"), rs.getString("email"),
								rs.getString("password"), dbRole, rs2.getString("mobile_number"),
								rs2.getString("address"), rs2.getInt("age"), childIds));
					}
				}
			}
