...
public class EventDAO {
    ...
    public boolean deleteEvent(long id) {
        try {
            Connection con = DatabaseConfig.getConnection();
            // Delete registrations first to avoid FK violation
    ...
    // Returns true if student is already registered for event
    ...
    public boolean registerStudent(long eventId, long studentId) {
        try {
            ...
            if (rs.next()) {
                ...
                if (cur >= max) {
                    System.out.println("Event is full! No more spots available.");
                    return false;
                }
            }
