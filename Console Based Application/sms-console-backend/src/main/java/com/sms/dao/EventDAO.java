...
//Author: Shobana V
/*
* This class for the query logic of an even based on create, update, select and delete event
*/
...
public class EventDAO {
	...
	private Event mapEvent(ResultSet rs) throws Exception {
		return new Event(rs.getLong("id"), rs.getString("event_name"), rs.getString("event_description"),
				rs.getString("event_date"), rs.getString("event_time"), rs.getString("event_location"),
				rs.getString("organizer"), rs.getInt("max_participants"), rs.getInt("current_participants"),
	...
	public Event getEventById(long id) {
		try {
			...
			if (rs.next())
				return mapEvent(rs); // found event
	...
	public boolean isRegistered(long eventId, long studentId) {
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("SELECT 1 FROM registrations WHERE event_id = ? AND student_id = ?");
	...
	public boolean registerStudent(long eventId, long studentId) {
		try {
			...
			PreparedStatement psCheck = con
					.prepareStatement("SELECT max_participants, current_participants FROM events WHERE id = ?");
			...
			PreparedStatement ps = con.prepareStatement(
					"INSERT INTO registrations (event_id, student_id, registration_date) VALUES (?, ?, ?)");
			ps.setLong(1, eventId);
			ps.setLong(2, studentId);
			...
			PreparedStatement psUpd = con
					.prepareStatement("UPDATE events SET current_participants = current_participants + 1 WHERE id = ?");
